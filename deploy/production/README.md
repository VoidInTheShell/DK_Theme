...
**CI deployment is suspended (since 2026-09-15):** the production deploy job on `main` is hard-disabled with a `false &&` guard; pushes and manual dispatches only build, test, and publish releases, and JPGREEN is updated through the panel's Admin version-update UI (or MCP update tasks). The trusted host scripts in this directory are retained as the documented manual emergency/recovery path. Historically, the `main` workflow could deploy production by manual dispatch, passing the run-specific `build-<run_id>-<run_attempt>` version tag through the production host's root-owned forced-command dispatcher. It updated only `xboard-theme`, used the panel deployment lock, and left `xboard-app`, `xboard-admin`, SQLite data, Xboard-Node and adjacent services untouched. After replacing Theme it validated the dynamic standalone-Admin route and gracefully reloaded BunkerWeb so the generated upstream immediately followed the new container IP; the BunkerWeb container was not restarted. The restricted `xboard-ci` user cannot upload or execute arbitrary scripts and is not a Docker-group member.
